...
export function PlayerControls({
  ...
  const handleSeekAction = useCallback(
    (value: number) => {
      ...
      // Трекаем событие
      if (value < 0) {
        trackSeekBackward(Math.abs(value), (currentProgress / duration) * 100)
      } else {
        trackSeekForward(value, (currentProgress / duration) * 100)
      }
      ...
      {isSong && (
        <>
          ...
          <PlayerButton
            ...
            tooltip={isLiked ? "Удалить из любимых" : "Добавить в любимые"}
          ...
          <PlayerButton
            ...
            data-testid="player-button-dislike"
            tooltip="Дизлайк (пропустить трек)"
          >
            <ThumbsDown className="text-secondary-foreground" />
          </PlayerButton>
        </>
      )}
    </div>
