  ...
  const starMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      if (!variables.starred) {
        ...
        initializeFromFavorites([{ id: artist.id, name: artist.name }])
        console.log(`[ML] ❤️ Artist "${artist.name}" liked! Added to preferredArtists with weight 5`)
      } else {
        // Разлайкнули → можно удалить или уменьшить вес (пока не трогаем)
        console.log(`[ML] 💔 Artist "${artist.name}" unliked`)
      }
    },
  })
