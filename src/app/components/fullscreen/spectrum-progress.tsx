...
interface SpectrumProgressProps {
  progress: number
  duration: number
...
export function SpectrumProgress({ progress, duration, onSeek, className }: SpectrumProgressProps) {
  ...
  // Генерируем столбики как в thunderdrome
  const bars = useMemo(() => {
    const barCount = 40
    return Array.from({ length: barCount }, (_, i) => {
      ...
      const height = Math.sin(barProgress * Math.PI) * 0.8 + 0.2
      
      // Цвет как в thunderdrome
      let color
  ...
  return (
    <div
      className={cn('relative w-full h-12 cursor-pointer flex items-end gap-px', className)}
      onClick={handleClick}
    >
      {bars.map((bar) => (
        <div
          key={bar.key}
          className="flex-1 rounded-t transition-opacity duration-200"
          style={{
            height: `${bar.height * 100}%`,
            backgroundColor: bar.color,
            opacity: bar.isPassed ? 1 : 0.4,
          }}
        />
      ))}
