...
interface ImageHeaderProps {
  ...
  badges: BadgesData
  isPlaylist?: boolean
  directImageUrl?: string // Прямая ссылка на изображение
  bannerUrl?: string // URL баннера (фон для шапки)
  logoUrl?: string // URL логотипа артиста
  actions?: React.ReactNode // Кнопки действий (радио, подписка и т.д.)
  onCoverClick?: () => void // Клик по обложке
}
...
export default function ImageHeader({
  ...
  return (
    <ImageLoader id={coverArtId} type={coverArtType} size={coverArtSize}>
      {(src, isLoading) => {
        // Используем прямую ссылку если она есть
        const imageSrc = directImageUrl || src
        
        return (
          <div
            className="flex relative w-full h-[calc(3rem+200px)] 2xl:h-[calc(3rem+250px)]"
            key={directImageUrl ? `header-url` : `header-${coverArtId}`}
          >
          ...
          >
            {/* Градиент поверх баннера */}
            <div
              className={cn(
                'absolute inset-0',
                'bg-gradient-to-b from-background/60 to-background/80',
              )}
            />
            ...
            <div className="relative z-10 flex gap-4 w-full">
              ...
              >
                <LazyLoadImage
                  key={directImageUrl ? 'url' : coverArtId}
                  effect="opacity"
                  ...
                  onError={handleError}
                  onClick={() => onCoverClick ? onCoverClick() : setOpen(true)}
                />
              </div>

              {/* Информация */}
              <div className="flex w-full max-w-[calc(100%-216px)] 2xl:max-w-[calc(100%-266px)] flex-col justify-end">
