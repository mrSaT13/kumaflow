...
import { PopoverVolume } from '@/app/components/player/popover-volume'
    ...
    <SimpleTooltip text={settings.enabled ? 'Авто-микс включен' : 'Авто-микс выключен'}>
      <Button
        ...
        className={cn(
          buttonsStyle.secondary,
          buttonsStyle.removeRing,
          settings.enabled && buttonsStyle.activeDot,
        )}
        style={{ ...buttonsStyle.style }}
        onClick={(e) => {
          e.stopPropagation()
      ...
      >
        <Radio className={cn('w-4 h-4', settings.enabled ? 'text-primary fill-current' : 'text-secondary-foreground')} />
      </Button>
    </SimpleTooltip>
  )
}

export function MiniPlayerVolumeButton() {
  const volume = usePlayerStore((state) => state.playerState.volume)
  ...
  return (
    <PopoverVolume>
      <Button
        ...
        style={{ ...buttonsStyle.style }}
      >
        <VolumeIcon volume={volume} size={16} />
      </Button>
    </PopoverVolume>
  )
}
